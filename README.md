...
This fork can also run as a small web service. It keeps the downloader as the core engine, but adds:

- a browser form for queueing jobs
- background workers so downloads do not block the web request
- per-job status pages with progress logs
- downloadable artifacts for completed jobs
- JSON file-list exports when using list mode

Start the service from the repository root:

    ruby bin/wayback_machine_server

By default it listens on `http://127.0.0.1:4567`.

If you are running a newer Ruby where WEBrick is no longer bundled, install it first:

    gem install webrick

Optional server flags:

    ruby bin/wayback_machine_server --host 0.0.0.0 --port 4567 --storage ./storage --workers 2
...
- HTML cleanup for base/canonical/archive-only markup
- from/to timestamps
- include/exclude filters
- include redirects and error responses
- concurrency
- snapshot page limit
- list-only JSON export

It still does not implement the heavier post-processing features that some commercial archive services provide, such as CMS conversion or large-scale semantic content repair, but it now includes a post-processing pass for:

- rewriting internal HTML and CSS URLs into portable static paths
- renaming query-string file paths into hostable filenames
- removing base tags and canonical tags that interfere with hosting the recovered site on a different domain
...
## Using the Web Docker Image

This fork also includes `Dockerfile.web` for the browser-based service:

    docker build -f Dockerfile.web -t wayback-machine-web .
    docker run --rm -p 4567:4567 -v $PWD/storage:/build/storage wayback-machine-web
